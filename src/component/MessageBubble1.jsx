...
mutation MyMutation($fromUserView: Boolean!, $_eq: uuid!) {
  update_groupMessages(where: {id: {_eq: $_eq}}, _set: {fromUserView: $fromUserView}) {
    affected_rows
  }
}
`
...
mutation MyMutation($_eq: uuid!, $toUserView: Boolean!) {
  update_groupMessages(where: {id: {_eq: $_eq}}, _set: {toUserView: $toUserView}) {
    affected_rows
  }
}
`
...
mutation MyMutation($_eq: uuid = "", $messages: String = "") {
  update_messages(where: {id: {_eq: $_eq}}, _set: {message: $messages}) {
    affected_rows
  }
}
...
function MessageBubble1(props) {
  ...
  const [deletePermanentGroupMessage] = useMutation(DELETE_PERMANENT_GROUP_MESSAGE)
    ...
    >
      ...
      >
        ...
        <button
                            ...
                            onClick={()=>selectedUser?.type ==="user"? isMe?handleDeleteOwnedMessage(message?.id):handleDeleteMessage(message?.id)
                            :isMe?handleDeleteOwnedGroupMessage(message?.id):handleDeleteGroupMessage(message?.id)
                          }
                          ...
                          </button>
                          {
                            ...
                            rel="noopener noreferrer"
                            style={{ color: "#1BA0E6" }}
                            onClick={()=>selectedUser?.type === "user"? handleDeletePermanent(message?.id):handleDeletePermanentGroupMessage(message?.id)}
                          >
                            Delete Permanent
                          </button><button
                            className="btn btn-light"
